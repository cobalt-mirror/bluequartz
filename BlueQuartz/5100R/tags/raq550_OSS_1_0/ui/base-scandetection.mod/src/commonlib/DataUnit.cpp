...
$header$
                ...
                Module :  $Source: /home/cvs/base-scandetection.mod/src/commonlib/DataUnit.cpp,v $
...
void DataUnit::readExternal (ObjectInput& objInput)
{
    int version = objInput.readInt ();

    if (version != serialVersionId)
    {
	throw IOException ("DataUnit::readExternal(): invalid serial version");
    }

    _data = objInput.readString();
}

void DataUnit::writeExternal (ObjectOutput& objOutput) const
{
    objOutput.writeInt (serialVersionId);
    objOutput.writeString (_data);
}
