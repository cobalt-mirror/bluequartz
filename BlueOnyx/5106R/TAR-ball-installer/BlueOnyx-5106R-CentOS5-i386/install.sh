...
rpm -hUv $RPM_PATH/blueonyx-yumconf-1.0.0-4.i386.rpm
rpm -hUv $RPM_PATH/solarspeed-blueonyx-repo-1.0.0-SOL2.i386.rpm

# If we're in an OpenVZ VPS (and *only* then!), we need the virtual 5106R YUM repository file as well:
if [ -e "/proc/user_beancounters" ];then
	rpm -hUv $RPM_PATH/solarspeed-virtual-5106R-repo-1.0.1-SOL1.i386.rpm
	# And we also install the dummy-centos RPM as the user may not have the right one:
	rpm -hUv --force --nodeps $RPM_PATH/dummy-centos-5-6.0-SOL5.i386.rpm
fi
...
rpm -hUv --force --nodeps $RPM_PATH/blueonyx-cd-installer-5.2-20090212.i386.rpm
