...
$iam = '/base/dns/vsite_dns_add_mx.php';
if ($domauth != '') {
    $parent = "/base/dns/vsite_records.php?group=$group&domauth=$domauth";
}
else {
    $parent = "/base/dns/vsite_records.php?group=$group";
}

$cceClient = $serverScriptHelper->getCceClient();
$factory = $serverScriptHelper->getHtmlComponentFactory("base-dns", "/base/dns/vsite_dns_add.php?TYPE=MX&group=$group");
$transMethodOn="off";
...
if (intval($_TARGET) > "0") {
    ...
    $mx_domain_name_Field = $factory->getTextField("mx_domain_name", $systemObj['domainname'], 'r');
}
else {
    // If create a new record, we fill in the domain name from the parent record:
    $mx_domain_name_Field = $factory->getTextField("mx_domain_name", $domauth, 'r');
}
$block->addFormField(
